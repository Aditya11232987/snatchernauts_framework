...
# Syncs the local Wiki/ directory to the GitHub wiki repository via SSH.
...
#
# Usage:
#   scripts/sync-github-wiki.sh           # full sync (force-push new snapshot)
#   scripts/sync-github-wiki.sh dry-run   # show what would be done
...
main() {
  ...
  (
    ...
    git add -A
    git -c user.name="wiki-sync" -c user.email="wiki-sync@local" commit -q -m "docs: sync wiki from project Wiki/"
