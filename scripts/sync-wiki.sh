#!/usr/bin/env bash
# Universal wiki sync script - syncs local Wiki/ directory to both GitLab and GitHub wikis
set -euo pipefail
...
show_usage() {
    cat << EOF
Universal Wiki Sync - Deploy documentation to both GitLab and GitHub wikis

USAGE:
    scripts/sync-wiki.sh [OPTIONS]
...
EXAMPLES:
    scripts/sync-wiki.sh                    # Sync to both GitLab and GitHub
    scripts/sync-wiki.sh --dry-run          # Preview changes without syncing
    scripts/sync-wiki.sh --gitlab-only      # Sync only to GitLab
    scripts/sync-wiki.sh --github-only      # Sync only to GitHub

REQUIREMENTS:
    - SSH access configured for both GitLab and GitHub
    - Wiki repositories enabled on both platforms
    - Local Wiki/ directory with documentation files
...
sync_to_wiki() {
    ...
    (
        ...
        if ! git diff --cached --quiet; then
            ...
            local commit_msg="docs: sync wiki documentation from project Wiki/ directory
...
main() {
    ...
    if [[ "$dry_run" == "true" ]]; then
        ...
        info "Would sync Wiki/ directory contents:"
